...
public class Parser {
	...
	private int excntCount = 0;
	private int[] lineCount = new int[100];
    ...
    private void program() {
		...
		gcprint("int excntArray[100] = {0};");
		gcprint("int excntLineArray[100] = {0};");
		gcprint("void excntUpdate(int index){excntArray[index] += 1;};");
		...
		if(excntCount > 0) {
			...
			for(int i = 0; i < excntCount; i++) {
				gcprint("printf(\"%4d%5d%9d\\n\"," + (i+1) + "," + lineCount[i] + "," + "excntArray["+ i + "]);");
			}
		}
...
//        currAssign = id;
	...
	private void edump() {
		int dumpLine = tok.lineNumber;
		mustbe(TK.DUMP);
		if(is(TK.NUM)){
			int dumpNum = Integer.parseInt(tok.string);
			if(dumpNum > symtab.getLevel()){
				warning("dump statement level (" + dumpNum + ") exceeds block nesting level (" + symtab.getLevel() +  "). using " + symtab.getLevel());
				dumpNum = symtab.getLevel();
			}
			
				gcprint("printf(\"+++ dump on line " + dumpLine + " of level " + dumpNum + " begin +++\\n\");");
				Stack<ArrayList<Entry>> table = symtab.getSymTab();
				for( ArrayList<Entry> block: table) {
				    for (Entry entry: block) {
						if(entry.getLevel() == dumpNum)
							gcprint("printf(\"%12d %3d %3d %s\\n\"," + "x_" + entry.getName() + "," + entry.getLineNumber() + "," + entry.getLevel() + "," + "\"" + entry.getName() + "\");" );
					}	
				}
				gcprint("printf(\"--- dump on line " + dumpLine + " of level " + dumpNum + " end ---\\n\");");
			
			mustbe(TK.NUM);
		}
		else{
			gcprint("printf(\"+++ dump on line " + dumpLine + " of all levels begin +++\\n\");");
			Stack<ArrayList<Entry>> table = symtab.getSymTab();
			for( ArrayList<Entry> block: table) {
			    for (Entry entry: block) {
					gcprint("printf(\"%12d %3d %3d %s\\n\"," + "x_" + entry.getName() + "," + entry.getLineNumber() + "," + entry.getLevel() + "," + "\"" + entry.getName() + "\");" );
			...
			gcprint("printf(\"--- dump on line " + dumpLine + " of all levels end ---\\n\");");
		}
		
	}
	
	private void eexcnt() {
		gcprint("excntUpdate(" + excntCount + ");");
		gcprint("excntLineArray[" + excntCount + "] = " + tok.lineNumber + ";");
		if(excntCount < 100) {
			lineCount[excntCount] = tok.lineNumber;
		}
		excntCount++;
		if(excntCount > 100){
